...
| A11 | **剩下的无名 `unreachable`，按「脚本能不能走到」分两堆** | 2026-08-30 晚数了 38 处。**脚本走得到的**（每个都该有名字或有答案）：(a) `"" + {}` / `o[{}]`——对象/函数的 ToString 没做（`runtime.rs` `to_string`，7.1.1 ToPrimitive 无原型），ECMA 的答案是 `"[object Object]"`，**该给答案不该 trap**；(b) `split("")` 的孤立代理项（`method.rs` `split`，能力树说的第四类故障）；(c) `slice` 的参数守卫（`slice_core` 两处，非 Number 下标）；(d) 数组方法落在非数组上（`unbox_array_from`、`array.rs` `prop_set` / `arr_set` 三处）。**引擎自证的**（脚本造不出来，留 trap 但可加 fault 码「不是这个引擎造的对」）：`to_number` / `truthy` / `type_of` / `str_cmp` 的「没有第九个 tag」、`repr` 的手造对守卫、`length` / `__call_check` 门控关掉时的空身体 | (a) 实现 ToString(Object) 与 ToString(Function)（`[object Object]` / `function`）；(b)(c)(d) 各给 fault 码与句子，有通道的走 TypeError；引擎自证的一律 `FAULT_ENGINE = 9` + 位置串。做完：每个 `Unreachable` 前面都有 `store_fault` 或一句「脚本到不了」的注释，`grep` 数得出来 |
