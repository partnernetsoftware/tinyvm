...
| A7 | 嵌套闭包 + 调用 `import` 进来的函数 → wasm 校验失败 | 下游第一波迁移六组之一测到：`loading wasm: validation: type mismatch`；顶层函数与顶层 try 没事，入口因此都写成平的 | **11 种形状都过**（返回的闭包、`map` 回调、`try` 内、两层嵌套、箭头、字符串参数——`tests/closures_call_imports_m3.rs`，2026-08-29）；报告里没有原始源码，暂不能复现。谁再撞到，把那段脚本贴进来。**2026-08-30 晚补的下半截**：再撞到时不用再二分——`Module::from_bytes_explained` 在校验函数体失败时答出**是哪个函数**（索引 + `name` 段里的名字；编译器早就把每个函数的名字写进 `name` 段了，只是没人读），`LoadError` 显示为 `validation: type mismatch in function \`broken\` (#2)`；旧入口的 `WasmError` 原样不动（`Copy`、无分配）；`tests/explained_load.rs`。**再补行号（同日深夜）**：编译器在 `name` 段旁边写一个 `qjs.lines` 自定义段（函数索引 → 1-based 源码行，词法器按 ECMA-262 LineTerminator 数行，`import` 进来的模块按它自己的源码数），`from_bytes_explained` 读进 `FunctionSite.line`，显示成 `… in function \`broken\` (#2) (line 7)`；脚本本身与运行时函数不列（前者永远是第 1 行，后者作者打不开），所以没有函数的程序字节不变、有函数的每个 +17 起（`tests/lower_m2.rs` 独立读取器、`tests/explained_load.rs` 手拼段）；静态核不变。**列也答了（2026-08-30 深夜）**：`qjs.lines` 段每项从 `(index, line)` 变 `(index, line, column)`，列是该行内 1-based 的 UTF-16 码元数（编辑器的数法；词法器每个 token 带 `column`，`\r\n`/U+2028 之后重新数）；`FunctionSite.column`，句子读作 `in function \`broken\` (#2) (line 7, column 12)`；无函数的程序字节不变，有函数的每个函数 +1 B（fleet 库 +30）；静态核不变 101 256；`tests/explained_load.rs`、`lower_m2.rs`（含非 BMP 字符前的列） |
...
| ~~A12~~ | ~~产品脚本手写的数组 / 字符串工具，按需求数落地~~ **四批已落地（2026-08-31）** | 2026-08-31 数下游产品脚本（注释就写在调用点）：`Array.prototype.concat`（`cu-macos-smoke.qjs:277`，agenterm-cu 的 agent 也撞过）、`join`（:326）、数组 `indexOf` / `includes`（`rh_compat.qjs:215` 手写 `contains`）、`sort`（`prune_target_incremental.qjs:317` 与 `check.qjs:683` 两份手写归并）、字符访问 `charCodeAt` / `charAt` / `s[i]` / `substring`（`rh_compat.qjs:446`、`qualification.qjs:18`、`build-all.qjs:40`）、`JSON.stringify(v, null, space)`（`rh_compat.qjs:96`）、`Array.isArray`（`rh_compat.qjs:200`） | **第一批已落地（2026-08-31）**：`Array.isArray`（parser 折成 `x.__is_array()`，与 `Object.keys` 同一条路）+ 数组 `indexOf` / `includes`（与 String 同名：一个 prefab 按 tag 分派，调用点放行两种 tag——`Recv::StrOrArr`；String 调用者因此 +221 B，记在 `tests/array_methods_m3.rs`）+ `concat(x[, y])` + `join([sep])`；`tests/method_conformance.rs` 四条（isArray 13 行、indexOf/includes 25 行、concat 19 行、join 18 行）、`tests/array_methods_cost.rs`（未命中 52 / 55 步/元素，concat 28 步/元素，join 236 步/10 字节元素）、`tests/array_methods_m3.rs`（不点名的程序字节不动；每个方法的价目）；`[].concat()`（零参）与 `splice` 仍是 `NotAFunction`。**第二批（同日）**：`sort([cmp])`——自底向上归并（无递归，不吃调用深度），稳定（平局取左）、原地（答案落回记录自己的向量）、返回接收者；默认序 = ToString 后 `__str_cmp`（码元序，`[10, 9, 1]` → `[1, 10, 9]` 是规范）、undefined 殿后且比较器看不到它、比较器答 NaN 记 0、非函数比较器具名 `comparefn`（fault 8）；`tests/sort_cost.rs`：1 000 个数 + `(p, q) => p - q` **1 708 542 步**（171 / n·log n），1 000 个串默认序 **4 207 002 步**（422 / n·log n；大头是 `__str_cmp` 逐码元解码——给它加 ASCII 字节快路是下一层，`<` 同受益），同一输入的手写归并（`prune_target_incremental.qjs:317` 原样）13 793 093 步，**3.3×**——那两份手写归并（另一份 `check.qjs:683`）可以退役了，脚本本身未改；价目 948 / 1 116 B。**第三批（同日）**：`charCodeAt` / `charAt` / `substring(a[, b])` / 只读 `s[i]`——四个共用 `slice` 的码元核心（`slice_core`），核心与 `charCodeAt` 的走法都学会**八个 ASCII 字节一步**（`ascii_skip`，`__len` 的那个测试）：1 000 字符第 999 位 42.7 → **2.1 步/码元**，逐位 `charCodeAt` 全串 23.3M → **3.0M 步**（`slice` 同受益）；`charCodeAt` 落在代理对中间答那半边（是个数，无需拒绝）；`charAt` / `s[i]` / `substring` 落在那里 = `slice` 的同一句 `a slice boundary inside a surrogate pair`（`tests/refused_operations.rs` 五种拼法一句）；`s[i]`：整数键 → 那一码元，越界 / 负 / 小数 → `undefined`（10.4.3.5），其它键走原来的属性路（`s["length"]` 照答）；**`s[i]` 不是名字，门只能是「文本定不了是 String 的计算键读」**（`o["a"]` 与 `for…of` 折出的 ` i` 不算）——所以 `o[k]` / `a[i]` 的程序也付 +716 B（`arrays_m3` 那行搬了，理由写在行上），步数 `a[i]` 不付（数组臂在前）、`o[k]` +4；`tests/char_access_cost.rs`、`tests/char_access_m3.rs`；`rh_compat.qjs:446` 的 `ASCII_PRINTABLE` 表与 `ascii_bytes`、`qualification.qjs:18` 的字符走法、`build-all.qjs:40` 的 `tail_of` 可以退役，脚本未改。**第四批（同日，最后一批）**：`JSON.stringify(v, null, space)`——25.5.2 第 5–8 步原样：Number 截断后夹到 0..10 个空格、String 取前 10 个码元（第 11 个若是代理对的前半，整对不取——半个没有 UTF-8）、其它类型无缩进；gap 与深度放在序列化缓冲的头里（`JB_GAP` / `JB_DEPTH`，头 12 → 20 B），`__jb_nl` 无 gap 时什么都不写，所以紧凑答案逐字节不变；空对象 / 空数组仍是 `{}` / `[]`，被省略的属性不留空行；oracle 是按规范手写的 pretty（serde 的 `Value` 是排序 map，不能当插入序的证人）；`tests/json_space.rs` 五条 + `json.rs` 两条改（space 不再拒绝、`__json_stringify` 里 replacer 句只剩一处）；**`replacer` 非 `undefined`/`null` 仍具名拒绝**（`this engine does not support a JSON.stringify replacer yet`——回调或属性表都还没定价）；JSON 程序 +501 B（`closures_m3` / `arrays_m3` / fleet 库三行搬了）；`rh_compat.qjs:96` 的 `stringify_pretty` 可以真的 pretty 了，脚本未改。**A12 四批全部落地。** |
| A13 | **`rh_compat.qjs` 2026-08-31 仍手写的数学 / 位运算 / 格式化工具，按需求数落地** | 2026-08-31 数下游（`git grep` 调用行 / 文件，rh_compat 自身除外）：`int_div` **68 行 / 8 文件**、`int_of` 46 / 7、`only_chars` 14 / 6、`rfc3339_from_ms` 10 / 3、`parse_int` 6 / 4、`fnv1a64_hex` 2 / 2；`xor16` / `hex4` / `pad_left` / `floor_div` / `fnv_mul_prime` 只被 rh_compat 内部调（各 1–4 行）。缺的引擎件：位运算（`xor16`、`fnv1a64_hex` 的 16 位 limb 乘）、`Math.floor` / 整除（`int_div` / `floor_div`）、`parseInt`、`padStart`（`pad_left`）、`toString(16)`（`hex4`）；`ascii_bytes` / `only_chars` 用 `charCodeAt` 已能写，不加；`rfc3339_from_ms` 要 `Date`——**B 区**（10 行 / 3 文件，先不做） | **第一批已落地（2026-08-31）**：`& \| ^ ~ << >> >>>` + `&= \|= ^= <<= >>= >>>=`——ECMA-262 7.1.6 ToInt32 一个 prefab（`__m_to_int32`：NaN / ±∞ → 0，截断，`x − 2^32·floor(x / 2^32)` 模 2^32，全程精确），七个运算符各一个 prefab（`Me::BitAnd`…`BitNot`），扫描按运算符点名（精确门：文本里有 `&` 才付）；13.12 的优先级三级 `\| ^ &` 在 `&&` 与 `==` 之间、移位在关系与加法之间（`parse.rs` 所有梯级 +6）；`tests/bitwise_conformance.rs` 12 条（负数、>2^31、小数、NaN/∞、`>>>` 到 2^32−1、移位计数 mod 32、String/Boolean 经 ToNumber、复合赋值、优先级）、`tests/bitwise_cost.rs`（一个运算符 **45 步**，2^31 外 65；1 000 次 `xor16` 手写 **14 526 095** 步 vs `(h ^ i) & 0xffff` **267 079**，54×）、`tests/bitwise_m3.rs`（不点名的程序字节不动；`&` 171 B、`~` 169、第二个运算符 +48）；`return 1;` 10 198 不动。**第二批（同日）**：`Math` 十个函数 + `PI`/`E`——无 `Math` 对象：每个成员在 parser 折成保留方法名（`Math.floor(x)` = `x.__math_floor()`，与 `Object.keys` 同路），常量与 `min()`/`max()`/`min(x)` 折成字面量或 `+x`（零字节零步）；`floor`/`ceil`/`trunc`/`abs`/`sqrt` = ToNumber + 一条 wasm 指令（IEEE 就是规范），`round` 平局向 +∞（`x−floor(x)` 精确所以半界不会错）、答 0 带原符号，`min`/`max` 是 IEEE 2019 min/max（NaN 传染、−0<+0 恰是 21.3.2.25/24），`pow` 走 6.1.6.1.3 的特例表 + 整数指数平方求幂（负指数 = 正的倒数，±0/±∞ 顺乘除自然对表）；**正有限底 + 分数指数具名拒绝**（`a fractional Math.pow exponent`，fault 3——engine 不带 exp/log，不静默近似）；缺的成员**编译期**具名拒绝（没有 `Math` 记录可 miss，`Math.cos` → `this engine does not support \`Math.cos\` yet`）；声明自己的 `Math` 者用自己的；价目 floor 270 B / 25 步、round 332 / 44、sign 311 / 37、min 277 / 28、pow 744 / 152；`int_div` 手写 1 000 次 633 054 步 → `Math.trunc(a/b)` 209 869（3.0×）；`tests/math_conformance.rs` 10 条、`math_cost.rs`、`math_m3.rs`；`return 1;` 不动。**第三批（同日）**：`parseInt(s[, radix])` + `Number.isInteger` / `Number.isNaN`——parser 把 `parseInt(s)` 折成 `s.__parse_int_radix(undefined)`（一个 prefab，两种元数），19.2.5 整条：`WsWidth` 的空白表、一个符号、`0x`/`0X`（基数 16 或缺省时）、逐位 `v*R+d`（2^53 内精确，之外逐位正确舍入——`"99999999999999999999"` 与 V8 逐位一致）、无位 NaN、基数 ToInt32 后出 2..36 是 NaN；`isInteger`/`isNaN` 是类型测试不转换（`isNaN("abc")` false）；声明自己的 `parseInt` 者用自己的；`parse_int_is_not_silently_number`（守了一年的拒绝钉）退役，`parse_int_reads_a_prefix_in_the_given_radix` 接班（`tests/parse_int_m3.rs` 6 条）；价目 parseInt 1 073 B（带基数 1 082）、isInteger 287、isNaN 265；200 次手写 `parse_int`（replaceAll 删数字验证）2 211 082 步 → `parseInt` 135 279（16×）；`return 1;` 不动。**第四批（同日，最后一批）**：`toUpperCase`（小写 run 表编译时反转：重复目标取最小大写=Unicode 简单映射；ς→Σ 手补一行；ß/µ/titlecase 原样——一对一表拼不出 "SS"，分歧记档并钉在 `tests/upper_m3.rs`；两方向各自门控，19 194 vs 19 294 vs 双向 27 669 B）+ `padStart`/`padEnd`/`repeat`（StringPad 原样：整倍填充一次分配 + `slice_core` 截前缀——代理对中间是 slice 的同一句；缺省 " "；负 repeat 具名拒绝 fault 3；`pad_left` 手写 100 次 2 841 292 步 → `padStart` 227 579，12×）+ `toString(radix)`（整数逐位 `fmod`+商，`__rem` 精确、商与 V8 同一双精度走法——2^53 外逐位同 V8；2..36 外与非十进制分数具名拒绝；`(1e21).toString(16)` 与 V8 逐字节一致）+ `toFixed(0..100)`（**精确十进制**：f64_fields 取 m·2^e，bignum 算 round_half_up(m·10^f·2^e)——`(1.005).toFixed(2)`="1.00"、`(8.005)`="8.01"、`(4.35).toFixed(1)`="4.3" 全与 V8 一致因为都是双精度的真话；新 `__m_bn_div10` 是 bignum 第一个除法；0..100 外具名拒绝；NaN/≥1e21 走 ToString）+ `n.toString()`（零参 = ToString 整条）；Recv 新增 `Num`（`(255).toString(16)` 走 prefab，`o.toString` 走属性路）；`missing_method` 的示例从 `padStart` 换 `normalize`；价目 toUpper 8 984（表）、padStart 1 635 / padEnd 1 633 / 缺省 1 666、repeat 669、toString(radix) 938、toString() 363、toFixed 1 243 B；`return 1;` 10 198 不动。**A13 四批全部落地（`rfc3339_from_ms` 的 `Date` 留在 B 区，10 行/3 文件）。** |
...
| ~~`toUpperCase`~~ **已落地（2026-08-31）** | 当年 to_upper 0 次；A13 复查后按镜像便宜落地（表反转，8 984 B 只有调用者付），A13 第四批 |
| ~~`parseInt`~~ **已落地（2026-08-31）** | 按名需求到了：`rh_compat.qjs` `parse_int` 下游 6 行/4 文件；19.2.5 整条落地，A13 第三批 |
...
> **区域式 bump（grok 评审 §3 的建议，2026-08-30；归因门已开，2026-09-04）**：不做追踪 GC（要给每个模块发射访问器，杀掉「不用不发射」），而是在 `JSON.parse` / `stringify` / 一次门答复投影完成后**恢复水位**——只有调这些操作的程序才带这段前置件，与逐方法预制件同一纪律。先量：哪些旅程的堆是「垃圾」而不是「活着的答复」，以及水位恢复点能否安全（答复投影后没有指针再指向被丢弃的中间记录）。现在已有显式 opt-in 的 diagnostic compile：只给实验模块追加只读总水位以及 `JSON.parse` / `JSON.stringify` 操作族累计分配计数，普通编译字节与脚本能力面不变；无持久 binding 且返回 primitive 的重复 JSON 调用已能给出「整段增量已死亡」的闭合证据。**操作族总分配不等于可回收后缀**：parse/stringify 都可能先分配临时前缀、再把活着的返回值放在堆尾；真实旅程仍须完成别名/末次使用归因后才有 L0 判决。尚未达到恢复水位或真实三旅程判决，不能把探针本身写成回收完成。
> **区域回收判决更新（2026-09-05）**：下游真实 server / wake
> 旅程已经完成 L0。JSON 操作占总分配 56.69% / 59.20%，但 parse 与
> stringify 都在临时前缀之后发布仍存活的返回记录，所以“操作返回即恢复
> 水位”的安全后缀是 0 B；该方案已判负，不再重开。下一候选只研究
> `JSON.stringify(binding)` 被单参数同步 host call 立即消费的更大
> producer→consumer 区域，并且必须先通过 ≥64 KiB、≥10%、至少两条真实
> 旅程的归因门；不达门槛就不写回收代码。
>
...
│   │   │   ├── 无声明形式 `for (x of y)`                   [x] 普通/成员目标；先取迭代值再求目标；`for_of_m3`
...
│   │   │   └── nullish coalescing `lhs ?? rhs`                  [x] exact slice; `nullish_coalescing`; no `??=`
│   │   │       ├── lhs once; rhs only for null/undefined; false/0/empty retained [x]
│   │   │       └── `&&`/`||` mix named refusal; no VM opcode; core 101 256 B; ordinary `||` golden unchanged [x]
...
│   │   ├── `Array.isArray(x)`：折成 `x.__is_array()`，任意接收者  [x] 2026-08-31；59 B；`rh_compat` 曾手写「有数字 length 的对象」
│   │   ├── 数组 `indexOf` / `includes`：与 String 同名，一个 prefab 按 tag 分派 [x] 2026-08-31；702 / 623 B；未命中 52 / 55 步/元素；String 调用者 +221 B
│   │   ├── `concat(x[, y])`：数组展开一层，其它原样追加        [x] 2026-08-31；569 / 639 B；28 步/元素
│   │   ├── `join([sep])`：undefined/null 为空，对象元素具名拒绝  [x] 2026-08-31；871 / 885 B；两趟一次分配，10 字节元素 236 步/个
│   │   ├── `sort([cmp])`：自底向上归并，稳定、原地、返回自身，undefined 殿后 [x] 2026-08-31；948 / 1 116 B；1 000 个数 + 比较器 1.71M 步，1 000 个串默认序 4.21M 步（手写归并 13.8M，3.3×）
│   │   ├── `charCodeAt(i)`：UTF-16 位置，代理项的半边是个数，照答     [x] 2026-08-31；733 B；ASCII 八字节一步，1 000 字符第 999 位 2 126 步
│   │   ├── `charAt(i)` / `substring(a[, b])`：与 slice 共一个码元核心    [x] 2026-08-31；869 / 1 026 / 909 B；落在代理对中间 = slice 的同一句拒绝
│   │   ├── `s[i]`（只读）：键是整数就是那一码元，越界 undefined      [x] 2026-08-31；768 B；门是「文本定不了的计算键读」，`o[k]` 也付（+716，记在账上）
│   │   ├── `JSON.stringify(v, null, space)`：数 1..10 / 串前 10 码元，25.5.2 原样 [x] 2026-08-31；JSON 程序 +501 B；50 个小对象 space=2 281 965 步（61/输出字节），紧凑路 178 202 不动；replacer 仍具名拒绝
...
│   │   ├── `Math`：floor/ceil/round/trunc/abs/sqrt/sign/pow/min/max + PI/E，逐名折成门控 prefab [x] 2026-08-31；无 `Math` 对象——常量与 0/1 参 min/max 折成字面量零字节；floor 25 步 / 270 B；`int_div` 循环 633k → `Math.trunc(a/b)` 210k 步；缺的成员编译期具名拒绝；pow 的正底分数指数具名拒绝（exp/log 未带）
│   │   ├── `toUpperCase`：小写表反转 + ASCII −32              [x] 2026-08-31；ς→Σ 手补；ß/µ/四个 titlecase 原样（记录的分歧，简单映射如此）；8 984 B 只有大写者付，两表各自门控
│   │   ├── `padStart` / `padEnd`（+ `repeat`）                [x] 2026-08-31；StringPad 原样：整倍 + slice_core 截前缀（代理对中间 = slice 的同一句拒绝）；缺省填充 " "；负 repeat 具名拒绝；1 635 / 1 633 / 669 B；100 次 pad_left 手写 2.84M → 228k 步（12×）
│   │   ├── `toString(radix)`：整数逐位除基，V8 同一走法        [x] 2026-08-31；2..36 外具名拒绝（RangeError）；非十进制下的分数具名拒绝不近似；938 B；`hex4` 可退役
│   │   ├── `toFixed(0..100)`：精确十进制（bignum），不是 %f     [x] 2026-08-31；`(1.005).toFixed(2)`="1.00" 而 `(8.005)`="8.01"（都是双精度的真话）；平局远离零（规范第 6 步先取 |x|）；1e21 起走 ToString；1 243 B
│   │   ├── 位运算 `& | ^ ~ << >> >>>` 与六个复合赋值：ToInt32 一个 prefab，逐运算符门控 [x] 2026-08-31；每个运算符 ~170 B、45 步（2^31 外 65）；`xor16` 手写 16 轮 14.5M → `^` 267k 步（54×）
...
│   │   │   └── `parseInt`（前缀解析 + 基数）                [x] 2026-08-31；19.2.5 整条：空白、一个符号、`0x`、逐位到第一个非位、无位 NaN、基数出 2..36 是 NaN；手写 parse_int 2.21M → 135k 步（16×）；1 073 B
│   │   │   └── `Number.isInteger` / `Number.isNaN`（21.1.2.3/.4）  [x] 2026-08-31；类型测试不转换（`isNaN("abc")` 是 false）；287 / 265 B
...
│   │   ├── split [x] · toLowerCase [x] · toUpperCase [x]  2026-08-31 三个都已落地
...
│   │   └── a refusal names the function, its name, its source line and column [x] `from_bytes_explained`：`name` 段 + `qjs.lines` 段（行 + UTF-16 列），静态核不读
│   └── bounded execution                                [~]
