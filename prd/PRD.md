...
> **区域式 bump（grok 评审 §3 的建议，2026-08-30；归因门已开，2026-09-04）**：不做追踪 GC（要给每个模块发射访问器，杀掉「不用不发射」），而是在 `JSON.parse` / `stringify` / 一次门答复投影完成后**恢复水位**——只有调这些操作的程序才带这段前置件，与逐方法预制件同一纪律。先量：哪些旅程的堆是「垃圾」而不是「活着的答复」，以及水位恢复点能否安全（答复投影后没有指针再指向被丢弃的中间记录）。现在已有显式 opt-in 的 diagnostic compile：只给实验模块追加只读 `__tinyvm_qjs_heap_ptr()`，普通编译字节与脚本能力面不变；无持久 binding 且返回 primitive 的重复 JSON 调用已能给出「整段增量已死亡」的闭合证据。尚未达到恢复水位或真实三旅程判决，不能把探针本身写成回收完成。
