...
| A9 | V1 装箱下的步数价格 | **实测（2026-08-29，下游 CLI 二分 `--max-operations`，扣除空程序 101 步）**：循环一次 **146**；`"" + n` **≈5 200**；`s = s + "x"`（串在长）**≈8 800**；`includes` **≈127/字符**；`JSON.parse` **≈520/字节**；`JSON.stringify` **≈700/字节**；`slice(0,10)` 于 1 000 字符 78 000 → `83721d0` 后 <3 000 | 表已进 PRD；`num_to_string` **已动**（`27d67b4`：整数走位数循环，537 步，下游 CLI 复测 ≈400）；`JSON.parse` **已动**（`json_pnum` 整数一趟：1 600 → 527 步/个；`json_pstr` 字符串四字节一步：119 → 29 步/字节；`plan/design-json-parse-fast.md`）；小数、短键、`__jp_at` 每位一次是下一层 |
