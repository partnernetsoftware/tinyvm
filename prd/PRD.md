...
| `break` | 17 | 21% | 56 | ✗ | 同名 |
| `continue` | 15 | 18% | 48 | ✗ | 同名 |
| `.replace(` | 14 | 17% | **142** | ✗ | `replace` |
| spread `...` | 14 | 17% | 20 | ✗ | 同名 |
| `parse_int` / `to_int` | 7 | 9% | 17 | ✗ | `Number()` / `parseInt` |
| `.len()` | 4 | 5% | 6 | ✅ | `.length` |
| `switch` | 2 | 2% | 6 | ✗ | 同名 |
| `do { } while` · 三元 · 解构 · 默认参数 | **0** | 0% | 0 | — | — |
...
**下一项是 `.to_upper/.to_lower`（30%，67 次），而它要先量成本再决定。**
ASCII 版本便宜，但会在 `"İ".toLowerCase()`、`"ß".toUpperCase()` 上给错答案——
这是**需要一张判据表的决定**，不是一次实现。

**同时记两条「零使用」**：`do { } while`、三元、解构、默认参数在 82 个脚本里
**一次都没出现**。加上第一张表里的 `switch`（2 个脚本 6 次），
这五项是目录驱动会做、需求驱动应当一直往后排的。
