...
fn a_wrong_type_call_is_not_reported_as_heap_exhaustion() {
    ...
    // Named since 2026-08-30: not heap exhaustion, and not a bare trap.
    assert_eq!(
...
fn an_arrow_function_is_a_function_value() {
    ...
    number(
        "function take(g) { return g(4); } return take((a) => a);",
        4.0,
    );
}
...
fn the_whole_fleet_library_compiles_and_its_methods_are_reachable() {
    ...
    //   correction recorded at `emit::m1::Lower::accessor`: the gate is the
    //   program and not the node, because `a.length` is a Static key on an
    //   array.
    //
    ...
    assert_eq!(
        wasm.len(),
        25_052, /* +23 on 2026-08-29: thrown-message record in the epilogue; see arrays_m3 ; +23 more the same day: the fleet library reads static properties, so it carries the arm that names a missing String property (`FAULT_MISSING_STRING_METHOD`); +89 on 2026-08-30: the `qjs.lines` section lists every one of its functions' source lines, see arrays_m3; +355 on 2026-08-30 (late): the JSON parse fast paths, see closures_m3; +30 the same night: a column beside every listed line in `qjs.lines`, one byte a function; +191 on 2026-08-31: `__num_to_string` covers the safe-integer range, see arrays_m3; +158 the same day: `__obj_find`'s cheap miss, see arrays_m3 */
