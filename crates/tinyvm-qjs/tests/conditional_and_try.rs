...
fn a_property_read_off_undefined_is_a_throw_and_a_real_trap_still_is_not() {
    ...
    // (`substring` was the example until it landed on 2026-08-31, and
    // `padStart` until it landed the same day.)
    let mut trapped = instantiate(
        "let out = 0; try { const s = \"ab\"; out = s.normalize(); } catch (e) { out = 5; } return out;",
    );
    trapped
        .invoke_by_name("main", &Value::args(&[]))
