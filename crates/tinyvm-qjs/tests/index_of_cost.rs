...
fn steps(source: &str) -> u64 {
    ...
    let module = WasmModule::from_bytes_with(&wasm, Limits { max_steps: 4_000_000_000, ..Limits::default() }).expect("loads");
    let mut instance = module.instantiate().expect("instantiates");
    instance.invoke_by_name("main", &Value::args(&[])).unwrap_or_else(|e| panic!("trap in {source:?}: {}", e.message()));
    instance.last_steps()
}

const BUILD: &str = r#"let s = "0123456789abcdef"; for (let i = 0; i < 13; i = i + 1) { s = s + s; }"#;
...
fn a_miss_skips_clear_windows() {
    ...
    assert!(per_inc < 10.0, "includes miss cost {per_inc:.1} steps a character; it was ~36");
    assert!(per_idx < 10.0, "indexOf miss cost {per_idx:.1} steps a character; it was ~39");
}
...
fn hits_and_misses_stay_exact_around_the_windows() {
    for (source, want) in [
        ...
        (r#"return "xxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxx".indexOf("y");"#, -1.0),
        (r#"return "héllo wörld".indexOf("wö");"#, 6.0),
        (r#"return "abc".indexOf("");"#, 0.0),
        (r#"return "ab".indexOf("abc");"#, -1.0),
        (r#"let t = ""; for (let i = 0; i < 100; i = i + 1) { t = t + "0123456789"; } t = t + "needle"; return t.indexOf("needle");"#, 1000.0),
        (r#"let t = ""; for (let i = 0; i < 100; i = i + 1) { t = t + "0123456789"; } return t.indexOf("9012") ;"#, 9.0),
    ] {
    ...
    for (source, want) in [
        ...
        (r#"return "xxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxx".includes("y");"#, false),
        ...
        let mut instance = module.instantiate().expect("instantiates");
        let vals = instance.invoke_by_name("main", &Value::args(&[])).expect("runs");
        assert_eq!(Value::returned(&vals).expect("value"), Value::Bool(want), "{source}");
    }
}
