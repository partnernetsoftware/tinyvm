...
fn fractions_still_take_the_general_path_and_still_answer() {
    ...
    assert_eq!(Value::returned(&vals).expect("value"), Value::Number(12345678901234568.0));
}
...
fn plain_ascii_strings_are_copied_in_runs() {
    ...
    let build = steps(r#"let t = "\""; for (let i = 0; i < 100; i = i + 1) { t = t + "0123456789"; } t = t + "\""; return t.length;"#);
    let parse = steps(r#"let t = "\""; for (let i = 0; i < 100; i = i + 1) { t = t + "0123456789"; } t = t + "\""; return JSON.parse(t).length;"#);
    let per_byte = (parse - build) / 1002;
    println!("JSON.parse of a 1000-char string: {per_byte} steps per byte");
    assert!(per_byte < 40, "a plain string byte cost {per_byte} steps; it was ~119");
}
