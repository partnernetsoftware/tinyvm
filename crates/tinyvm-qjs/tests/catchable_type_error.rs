...
fn a_finally_runs_and_the_script_continues_after_the_catch() {
    assert_eq!(
        text(r#"let log = ""; try { let u = undefined; u.a; } catch (e) { log = log + "c"; } finally { log = log + "f"; } return log + "!";"#),
        "cf!"
    );
}
...
fn without_a_try_the_read_is_still_the_named_fault() {
    ...
    assert_eq!(guest_fault(&instance.memory().expect("guest memory")), Some(GuestFault::PropertyOfNonObject));
}
