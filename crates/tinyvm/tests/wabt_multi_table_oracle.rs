...
#[test]
#[ignore = "run through smoke-wabt-multi-table.sh with an independently compiled fixture"]
fn wabt_compiled_multi_table_matches_tinyvm() {
    let path = PathBuf::from(
        std::env::var_os("TINYVM_WABT_MULTI_TABLE_WASM")
            .expect("TINYVM_WABT_MULTI_TABLE_WASM is set by the smoke script"),
    );
