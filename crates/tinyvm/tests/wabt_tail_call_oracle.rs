...
#[test]
#[ignore = "run through smoke-wabt-tail-call.sh with an independently compiled fixture"]
fn wabt_compiled_tail_calls_match_tinyvm() {
    let path = PathBuf::from(
        std::env::var_os("TINYVM_WABT_TAIL_CALL_WASM")
            .expect("TINYVM_WABT_TAIL_CALL_WASM is set by the smoke script"),
    );
    ...
    let result = must_ok(instance.invoke_by_name("run", &[]), "run tail-call fixture");
