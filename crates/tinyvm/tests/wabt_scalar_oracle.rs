...
#[test]
#[ignore = "run through smoke-wabt-scalar-proposals.sh with an independently compiled fixture"]
fn wabt_compiled_scalar_proposals_match_tinyvm() {
    let path = PathBuf::from(
        std::env::var_os("TINYVM_WABT_SCALAR_WASM")
            .expect("TINYVM_WABT_SCALAR_WASM is set by the smoke script"),
    );
    ...
    let result = must_ok(instance.invoke_by_name("run", &[]), "run scalar fixture");
