...
struct MultiMemoryOracle {
    static func main() throws {
        ...
        if let javascriptError {
            guard javascriptError.contains("more than one memory") else {
                throw OracleError.javascript(javascriptError)
            }
            print("OK: JavaScriptCore reports standard multiple memories unsupported")
            return
        }
        let result = value?.toInt32() ?? Int32.min
        guard result == 1225 else { throw OracleError.wrongResult(result) }
        print("OK: JavaScriptCore standard multiple-memory result=1225")
    }
}
