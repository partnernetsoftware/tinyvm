    ...
    static func main() throws {
        ...
        let value = context.evaluateScript(
            ...
            (() => {
              ...
              const reductions = instance.exports.reductions();
              if (reductions !== 1) throw new Error(`reductions=${reductions}`);
              const laneBounds = instance.exports.lane_bounds();
              if (laneBounds !== 1) throw new Error(`lane_bounds=${laneBounds}`);
              const saturation = instance.exports.saturation();
              if (saturation !== 1) throw new Error(`saturation=${saturation}`);
              ...
              return `${added}|${subtracted}|mask=${any}|rearrange=pass|lanes=pass|comparisons=pass|reductions=pass|bounds=pass|saturation=pass|bridge=pass`;
            })()
            """
        )
        if let javascriptError { throw OracleError.javascript(javascriptError) }
        let result = value?.toString() ?? ""
        let expected = "32767,-32768,300,-300,32767,-32768,-5000,5000|20000,-20000,-100,100,32766,-32767,32767,-32768|mask=1,0|rearrange=pass|lanes=pass|comparisons=pass|reductions=pass|bounds=pass|saturation=pass|bridge=pass"
